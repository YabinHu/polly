...
extractArraysInfo(polly::Scop *S,
                  const std::set<const llvm::Value *> &ArrayBases) {
  ...
  for (const Value *BaseAddr : ArrayBases) {
    ...
    isl_space *Dim = isl_space_set_alloc(Ctx, 0, NumDims /*Depth, linearized*/);
    Dim = isl_space_set_tuple_id(Dim, isl_dim_set, ArrInfo->getBasePtrId());
